...
function showAnalyticsSkeleton() {
  ...
  // Chart skeletons
  document.getElementById('scansChart').innerHTML =
    '<div class="skeleton" style="height:160px;width:100%;"></div>';
  document.getElementById('labelsChart').innerHTML =
...
function renderScansPerDayChart(records) {
  ...
  html += '</div>';
  document.getElementById('scansChart').innerHTML = html;
}
...
function renderTopLabelsChart(labelCount) {
  ...
  if (!sorted.length) {
    document.getElementById('labelsChart').innerHTML =
      '<div class="empty-state" style="padding:24px;"><div>No labels yet</div></div>';
    return;
  }
...
function renderActivityTimeline(records) {
  if (!records.length) {
    document.getElementById('activityTimeline').innerHTML =
      '<div class="empty-state" style="padding:16px;"><div>No activity yet</div></div>';
    return;
  }
